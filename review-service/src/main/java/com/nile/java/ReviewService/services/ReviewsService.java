...
public class ReviewsService {
    ...
    public void addReview(String token, Review review) {
        ...
        // The order for which the review is being added is checked against it's buyer ID.
        // Validate Jwt
    ...
    public void deleteReview(String token, UUID id) {
        if (authenticateBuyer(token).matches("OK")) {
            ...
            // Only user that created the review can delete it.
            if (review.isPresent()) {
                if (review.get().getBuyerId().matches(getBuyerId(token))) {
